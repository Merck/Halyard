 ...
 * <ul>
 * <li>Frequency list is stored as an array with no next/prev pointers between nodes: looping over the array should be faster and more CPU-cache friendly than using an ad-hoc
 * linked-pointers structure.</li>
 * <li>The max frequency is capped at the cache size to avoid creating more and more frequency list entries, and all elements residing in the max frequency entry are
 * re-positioned in the frequency entry linked set in order to put most recently accessed elements ahead of less recently ones, which will be collected sooner.</li>
 * <li>The eviction factor determines how many elements (more specifically, the percentage of) will be evicted.</li>
 * </ul>
 ...
 * @author Sergio Bossa
 */
public class LFUCache<Key, Value> extends AbstractMap<Key, Value> {
