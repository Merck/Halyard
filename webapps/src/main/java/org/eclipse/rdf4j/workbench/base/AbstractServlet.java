...
public abstract class AbstractServlet implements Servlet {
	...
	protected QueryResultWriter getResultWriter(final HttpServletRequest req, final ServletResponse resp,
		...
		if (resultFormat == null) {
			...
			resultFormat = TupleQueryResultFormat.SPARQL_STAR;
		}
	...
	protected TupleResultBuilder getTupleResultBuilder(HttpServletRequest req, HttpServletResponse resp,
		...
		// Halyard changes: work-around for broken SPARQLStarResultsXMLWriterFactory
		if (resultWriter.getSupportedSettings().contains(BasicWriterSettings.ENCODE_RDF_STAR)) {
			resultWriter.getWriterConfig().set(BasicWriterSettings.ENCODE_RDF_STAR, false);
		}
