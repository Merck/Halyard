...
public final class ParallelSplitFunction implements Function {
     ...
     */
    public static final String PARALLEL_SPLIT_FUNCTION_NAME = "forkAndFilterBy";
    ...
    public Value evaluate(ValueFactory valueFactory, Value... args) throws ValueExprEvaluationException {
        if (args == null || args.length < 2) {
            throw new ValueExprEvaluationException(PARALLEL_SPLIT_FUNCTION_NAME + " function has at least two mandatory arguments: <constant number of parallel forks> and <binding variable(s) to filter by>");
        }
        for (Value v : args) {
            if (v == null) {
                throw new ValueExprEvaluationException(PARALLEL_SPLIT_FUNCTION_NAME + " function does not allow null values");
            }
        }
        try {
            int forks = Integer.parseInt(args[0].stringValue());
            if (forks < 1) {
                throw new ValueExprEvaluationException(PARALLEL_SPLIT_FUNCTION_NAME + " function first argument must be > 0");
            }
            return valueFactory.createLiteral(Math.floorMod(Arrays.hashCode(args), forks) == forkIndex);
        } catch (NumberFormatException e) {
            throw new ValueExprEvaluationException(PARALLEL_SPLIT_FUNCTION_NAME + " function first argument must be numeric constant");
        }
    }
    ...
    private static final class ParallelSplitFunctionVisitor extends AbstractQueryModelVisitor<IllegalArgumentException> {
        ...
        public void meet(FunctionCall node) throws IllegalArgumentException {
            if (PARALLEL_SPLIT_FUNCTION_URI.equals(node.getURI())) {
                List<ValueExpr> args = node.getArgs();
                if (args.size() < 2) throw new IllegalArgumentException(PARALLEL_SPLIT_FUNCTION_NAME + " function has at least two mandatory arguments: <constant number of parallel forks> and <binding variable(s) to filter by>");
                try {
                    ...
                    if (forks == 0) {
                        ...
                        throw new IllegalArgumentException(PARALLEL_SPLIT_FUNCTION_NAME + " function is used twice with different first argument (number of forks)");
                    }
                } catch (ClassCastException | NullPointerException | NumberFormatException ex) {
                    throw new IllegalArgumentException(PARALLEL_SPLIT_FUNCTION_NAME + " function first argument (number of forks) must be integer constant >0");
                }
            }
        }
    }
