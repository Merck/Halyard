...
public class HalyardParallelExportTest {
    ...
    public void testExport() throws Exception {
        ...
        assertEquals(0, ToolRunner.run(HBaseServerTestInstance.getInstanceConfig(), new HalyardParallelExport(),
                new String[]{"-s", "exportTable", "-q", "PREFIX halyard: <http://merck.github.io/Halyard/ns#>\nselect * where {?s ?p ?o .\nFILTER (halyard:forkAndFilterBy (2, ?p))}", "-t", root.toURI().toURL().toString() + "data{0}.csv"}));
    ...
    public void testRunVersion() throws Exception {
        assertEquals(0, new HalyardParallelExport().run(new String[]{"-v"}));
    }

    @Test(expected = UnrecognizedOptionException.class)
    public void testRunInvalid() throws Exception {
        new HalyardParallelExport().run(new String[]{"-invalid"});
    }
}
