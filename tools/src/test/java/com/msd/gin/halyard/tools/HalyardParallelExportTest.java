...
public class HalyardParallelExportTest {
    ...
    public void testExport() throws Exception {
        ...
        assertEquals(0, ToolRunner.run(HBaseServerTestInstance.getInstanceConfig(), new HalyardParallelExport(),
                new String[]{"-Dmapreduce.job.maps=2", "-s", "exportTable", "-q", "PREFIX hlyd: <http://gin.msd.com/halyard/>\nselect * where {?s ?p ?o .\nFILTER (hlyd:parallel_split_by (?p))}", "-t", root.toURI().toURL().toString() + "data{0}.csv"}));
