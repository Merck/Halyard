...
public class ParallelSplitFunctionTest {
    ...
    public void testGetURI() {
        assertEquals(ParallelSplitFunction.PARALLEL_SPLIT_FUNCTION_URI, new ParallelSplitFunction(1).getURI());
    }
    ...
    public void testGetNumberOfForksFromFunction() {
        assertEquals(5, ParallelSplitFunction.getNumberOfForksFromFunctionArgument("select * where {?s ?p ?o. filter <" + ParallelSplitFunction.PARALLEL_SPLIT_FUNCTION_URI + ">(5, ?s)}", false, 0));
    }
    ...
    public void testGetNumberOfForksNoArgs() {
        ParallelSplitFunction.getNumberOfForksFromFunctionArgument("select * where {?s ?p ?o. filter <" + ParallelSplitFunction.PARALLEL_SPLIT_FUNCTION_URI + ">()}", false, 0);
    }

    @Test(expected = IllegalArgumentException.class)
    public void testGetNumberOfForksNANArg() {
        ParallelSplitFunction.getNumberOfForksFromFunctionArgument("select * where {?s ?p ?o. filter <" + ParallelSplitFunction.PARALLEL_SPLIT_FUNCTION_URI + ">(\"not a number\", ?s)}", false, 0);
    }

    @Test(expected = IllegalArgumentException.class)
    public void testGetNumberOfForksVarArg() {
        ParallelSplitFunction.getNumberOfForksFromFunctionArgument("select * where {?s ?p ?o. filter <" + ParallelSplitFunction.PARALLEL_SPLIT_FUNCTION_URI + ">(?p, ?s)}", false, 0);
    }

    @Test(expected = IllegalArgumentException.class)
    public void testGetNumberOfForksNegativeArg() {
        ParallelSplitFunction.getNumberOfForksFromFunctionArgument("select * where {?s ?p ?o. filter <" + ParallelSplitFunction.PARALLEL_SPLIT_FUNCTION_URI + ">(-1, ?s)}", false, 0);
    }
    ...
    public void testGetNumberOfForksFromUpdate() {
        String query = "insert {?s ?p ?o} where {?s ?p ?o. filter <" + ParallelSplitFunction.PARALLEL_SPLIT_FUNCTION_URI + ">(5, ?s)};"
            + "clear all;"
            + "delete {?s ?p ?o} where {?s ?p ?o. filter <" + ParallelSplitFunction.PARALLEL_SPLIT_FUNCTION_URI + ">(8, ?s)}";
